    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Maaf, Minimal 10 members group untuk bisa menggunakan bot. Bye~').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Halo members group *${name}*, terimakasih telah invite bot ini, untuk menggunakan */menu*`)
                }
            }))
